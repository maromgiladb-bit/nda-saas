...
export default function SignNDASimplePage() {
  ...
  // Inject signature into HTML for live preview
  const injectSignatureIntoHtml = (html: string, signature: string | null, typedName: string) => {
    // Add Google Font link to head
    const fontLink = '<link href="https://fonts.googleapis.com/css2?family=Sacramento&display=swap" rel="stylesheet">';
    let modifiedHtml = html;
    if (!html.includes('family=Sacramento')) {
      modifiedHtml = html.replace('</head>', `${fontLink}</head>`);
    }

    let signatureHtml = '';

    if (signatureMode === 'type' && typedName) {
      // For typed signature, render as text with the font
      signatureHtml = `<div style="font-family: 'Sacramento', cursive; font-size: 48px; color: black; text-align: center;">${typedName}</div>`;
    } else if (signature) {
      // For drawn/uploaded, render as image
      signatureHtml = `<img src="${signature}" alt="Signature" />`;
    } else {
      return modifiedHtml;
    }

    if (modifiedHtml.includes('id="party-a-signature"')) {
      return modifiedHtml.replace(
        /(<div[^>]*id="party-a-signature"[^>]*>)(.*?)(<\/div>)/s,
        `$1${signatureHtml}$3`
      );
    }
  ...
  const liveHtmlContent = ndaData?.htmlContent
    ? injectSignatureIntoHtml(ndaData.htmlContent, signatureImage, typedSignature)
    : "";
  ...
  useEffect(() => {
    if (signatureMode === 'type' && typedSignature) {
      ...
      const ctx = canvas.getContext('2d');
      if (ctx) {
        ctx.font = `60px ${sacramento.style.fontFamily}`;
        ctx.fillStyle = "black";
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        ...
        {/* LEFT: PDF Preview (~60%) */}
        <div className="w-full lg:w-[60%] bg-white border-b lg:border-b-0 lg:border-r border-gray-200 overflow-y-auto h-[40vh] lg:h-auto">
          ...
          <div className="p-6">
            {generatingPreview ? (
              <div className="text-center py-20">
                <svg className="animate-spin h-12 w-12 mx-auto mb-4 text-[var(--teal-600)]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
        ...
        <div className="w-full lg:w-[40%] bg-gray-50 overflow-y-auto flex-1">
          <div className="p-8">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
              ...
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 bg-[var(--teal-50)] border border-[var(--teal-100)] rounded-lg px-3 py-2">
                    <div className="w-8 h-8 rounded-full bg-[var(--teal-100)] flex items-center justify-center text-[var(--teal-700)] font-bold text-sm">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
              ...
              <div className="flex p-1 bg-gray-100 rounded-lg mb-4">
                {(['type', 'draw', 'upload'] as const).map((mode) => (
                  <button
                    key={mode}
                    className={`flex-1 py-2 text-xs font-bold rounded-md transition-all capitalize ${signatureMode === mode ? 'bg-white text-[var(--teal-700)] shadow-sm' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-200/50'}`}
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 min-h-[200px]">
                ...
                {signatureMode === 'type' && (
                  <div className="space-y-3">
                    <input
                      ...
                      className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[var(--teal-600)] focus:border-[var(--teal-600)] outline-none transition-all bg-white"
                      placeholder="Type your full name..."
                    />
                    <div className="h-24 bg-white border border-dashed border-gray-300 rounded-lg flex items-center justify-center">
                      {typedSignature ? (
                        <p className={`${sacramento.className} text-5xl text-black`}>{typedSignature}</p>
                      ) : (
                ...
                {signatureMode === 'upload' && (
                  <div>
                    <div className="border border-dashed border-gray-300 rounded-lg p-8 text-center hover:bg-white hover:border-[var(--teal-600)] transition-all cursor-pointer relative group">
                      <input type="file" accept="image/*" onChange={handleFileUpload} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
                      <div className="flex flex-col items-center pointer-events-none">
                        <svg className="w-12 h-12 text-gray-300 group-hover:text-[var(--teal-600)] mb-2 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    {signatureImage && (
                      <div className="mt-3 border border-[var(--teal-100)] rounded-lg p-3 bg-[var(--teal-50)] flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-[var(--teal-700)] font-semibold">✓ Uploaded</span>
                        </div>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[var(--teal-600)] focus:border-[var(--teal-600)] outline-none transition bg-white"
                  />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
            ...
            <button
              ...
              className="w-full bg-[var(--teal-600)] text-white px-6 py-4 rounded-xl hover:bg-[var(--teal-700)] disabled:bg-gray-300 disabled:text-gray-500 disabled:cursor-not-allowed font-bold text-lg shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all flex items-center justify-center gap-3"
            >
              {loading ? (
                <>
